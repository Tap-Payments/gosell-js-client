...
class GoSell extends Component {
  ...
  static openPaymentPage() {
    ...
    if (RootStore.configStore.token != null) {
      window.open(CONFIGS.framePath + "?mode=" + RootStore.uIStore.modalMode + "&token=" + RootStore.configStore.token, "_self")
    }
  }
    ...
    eventer(
      ...
      function (e) {
        ...
        if (e.data == "close" || e.data.close) {
          ...
          e.data == "close" ? self.closeModal(RootStore.configStore.redirect_url) : self.closeModal(e.data.close)
        }
      },
      false
    )
  ...
  render() {
    return (
      <React.Fragment>
        {RootStore.uIStore.modalMode == "popup" && RootStore.uIStore.openModal ? (
          <iframe
            id='gosell-gateway'
            style={{
        ...
        {RootStore.uIStore.openModal ? (
          <TapLoader type='loader' color={"white"} store={RootStore} status={RootStore.uIStore.isLoading} duration={5} title={null} desc={null} />
        ) : null}
      </React.Fragment>
    )
  }
